...
async function setEditorCode(page: Page, nextCode: string): Promise<void> {
  const editorMode = await page.evaluate(() => {
    const shell = document.querySelector(".code-editor-shell")
    return shell && shell.getAttribute("data-editor-mode")
  })

  if (editorMode === "ace") {
    await page.evaluate((code: string) => {
      const aceHost = document.getElementById("starter-code-ace")
      const textarea = document.getElementById(
        "starter-code-editor"
      ) as HTMLTextAreaElement | null
      const aceWindow = window as Window & {
        ace?: {
      ...
      if (
        aceHost &&
        aceWindow.ace &&
        typeof aceWindow.ace.edit === "function"
      ) {
        const editor = aceWindow.ace.edit(aceHost)
        if (editor.session && typeof editor.session.setValue === "function") {
  ...
  await page.locator("#starter-code-editor").fill(nextCode)
}

async function runReferenceSolution(page: Page): Promise<void> {
  await setEditorCode(page, ATTENTION_REFERENCE_SOLUTION.trim())
  ...
  await expect
    .poll(async () => {
      return await page.evaluate(() => {
        ...
        return `${activeId} ${activeClass}`.trim()
      })
    })
    .toMatch(/starter-code-editor|ace_text-input/)
  await expect(page.locator(".code-editor-shell")).toHaveClass(/is-editing/)
  if (editorMode === "ace") {
    await expect(page.locator("#starter-code-ace")).toBeVisible()
  } else {
    await expect
      .poll(async () => {
        return await page.evaluate(() => {
