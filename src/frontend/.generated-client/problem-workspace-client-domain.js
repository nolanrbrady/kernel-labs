...
class QuestionCatalog {
    ...
    parseCatalog(rawCatalog) {
        ...
        try {
            ...
            return parsedCatalog
                ...
                var problemType = typeof entry.problemType === "string"
                    ? entry.problemType
                    : "Uncategorized";
                var summary = typeof entry.summary === "string"
                    ? entry.summary
                    : "Atomic toy-tensor coding problem.";
                var estimatedMinutes = typeof entry.estimatedMinutes === "number" &&
    ...
    filterQuestions(normalizedQuery, selectedType) {
        ...
        var safeType = typeof selectedType === "string" && selectedType.length > 0
            ? selectedType
            : "all";
        return this.catalog
    ...
    renderQuestionListHtml(questions) {
        return questions
            .map(function (question) {
            return ("<li class=\"question-library-item\">" +
...
class AnonymousProgressStore {
    ...
    persistAttempt(correctness) {
        ...
        if ((correctness === "pass" || correctness === "partial") &&
    ...
    getPriorSuccessfulCompletions(progress) {
        var targetProgress = isPlainObject(progress) && Array.isArray(progress.attemptHistory)
            ? progress
            : this.createEmptyProgress();
        return targetProgress.attemptHistory.reduce(function (count, attempt) {
...
export { QuestionCatalog, VisibleTestCaseTracker, SuggestTopicFormValidator, AnonymousProgressStore };
