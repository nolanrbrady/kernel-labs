...
(function () {
    ...
    var codeEditorShell = codeEditor && typeof codeEditor.closest === "function"
    ...
    function updateTimerDisplay(remainingMs) {
        ...
        setText(sessionTimerStatus, "Session timer: " + formatTimerClock(remainingMs) + " remaining.");
    }
    ...
    function maybeAutoSubmitAtCap() {
        ...
        if (timerCapMessage) {
            setText(timerCapMessage, "30 minutes reached. Submitting automatically so this session ends cleanly.");
        }
    ...
    function startSessionTimer(sourceLabel) {
        ...
        if (timerCapMessage) {
            setText(timerCapMessage, "Timer started. You can run as many experiments as you want before submit.");
        }
    ...
    function applyHintReveal(tier) {
        if (tier !== revealedHintTier + 1) {
            if (hintStatus) {
                setText(hintStatus, "Hints unlock in order. Start with the next available tier.");
            }
            return;
        }
        ...
        if (hintStatus) {
            if (tier === 3) {
                ...
                setText(hintStatus, "Hint tier " + tier + " revealed. You can still submit at any time.");
            }
        }
    }
    ...
    if (localProgress.completedProblemIds.indexOf(problemId) !== -1) {
        setText(sessionStatus, "Session status: active. Previous anonymous completion found for this problem.");
    }
