...
(function (globalScope) {
    ...
    class WorkspaceTabController {
        ...
        bind() {
            ...
            this.workspaceTabProblem.addEventListener("click", function () {
                this.activate("problem");
            }.bind(this));
            this.workspaceTabLibrary.addEventListener("click", function () {
    ...
    class VisibleTestCaseController {
        ...
        applyVisibleTestCaseState(statusByCaseId) {
            for (var index = 0; index < this.visibleTestCaseIds.length; index += 1) {
                ...
                if (status) {
                    setText(status, caseState && typeof caseState.statusLabel === "string"
        ...
        applyResults(results) {
            ...
            if (summary.totalCount > 0 && typeof this.appendDebugLine === "function") {
                this.appendDebugLine("> visible test cases: " +
                    summary.passedCount +
                    "/" +
                    summary.totalCount +
                    " passed.");
    ...
    class QuestionLibraryController {
        ...
        render() {
            ...
            var normalizedQuery = domain.QuestionCatalog.normalizeQueryText(this.questionSearchInput && typeof this.questionSearchInput.value === "string"
                ? this.questionSearchInput.value
                : "");
            var selectedType = this.getSelectedType();
            var filteredQuestions = this.catalogModel.filterQuestions(normalizedQuery, selectedType);
            setText(this.questionLibraryCount, "Showing " + filteredQuestions.length + " of " + this.catalog.length + " questions.");
            if (filteredQuestions.length === 0) {
                if ("innerHTML" in this.questionLibraryResults) {
                    ...
                    setText(this.questionLibraryResults, "No matching questions yet. Try a different keyword or type.");
                }
                return;
            }
            ...
            setText(this.questionLibraryResults, this.catalogModel.renderQuestionListText(filteredQuestions));
        }
        bind() {
