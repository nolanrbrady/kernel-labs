...
- [x] P1-05 | Robust Card Verification Pipeline | Build a strict verification workflow that validates question quality and reference-solution correctness before cards are eligible for scheduling. | blocked_by=none | verify=make test;make lint
  PRD refs: 6, 8, 13, 18
  Acceptance:
...
- [x] P2-24 | Animated Submission Success Moment | Add a lightweight celebratory success animation + copy treatment after submit to make completion feel rewarding without introducing guilt mechanics. | blocked_by=none | verify=make test;make lint
  PRD refs: 5, 18
  Acceptance:
...
- [x] P2-25 | Prominent Next Presentation Countdown | Make "days until next presentation" the primary spaced-repetition feedback signal in submit/done surfaces. | blocked_by=none | verify=make test;make lint
  PRD refs: 5, 9, 18
  Acceptance:
...
- 2026-02-17T15:54:43Z | P2-24 | files=src/frontend/problem-workspace-route.tsx,src/frontend/problem-workspace.css,src/frontend/client-ts/workspace-client/api/session-controllers.ts,src/frontend/client-ts/workspace-client/index.ts,tests/frontend/workspace/client-controllers.test.ts,tests/frontend/workspace/client-script.integration.test.ts,tests/editor-first-landing.route.test.tsx,tests/backend/start.entrypoint.integration.test.ts,TODO.md | tests=added/updated | verify=npm run test:unit;npm run lint;make test (unit pass, e2e blocked by Playwright sandbox);make lint | risks=celebration state is class-driven and currently scoped to the status panel; if future UI refactors move session status out of this panel, class wiring should be updated
- 2026-02-17T15:54:43Z | P2-25 | files=src/frontend/problem-workspace-route.tsx,src/frontend/problem-workspace.css,src/frontend/client-ts/workspace-client/api/session-controllers.ts,src/frontend/client-ts/workspace-client/index.ts,tests/frontend/workspace/client-controllers.test.ts,tests/frontend/workspace/client-script.integration.test.ts,tests/editor-first-landing.route.test.tsx,tests/backend/start.entrypoint.integration.test.ts,TODO.md | tests=added/updated | verify=npm run test:unit;npm run lint;make test (unit pass, e2e blocked by Playwright sandbox);make lint | risks=next-presentation date uses local runtime clock and ISO formatting; timezone-aware date presentation may be needed for multi-region UX copy later
- 2026-02-17T17:05:42Z | MAINT-ACE-EDITOR | files=src/frontend/client-ts/workspace-client/controllers/editor-controller.ts,tests/frontend/workspace/client-controllers.test.ts,docs/local-first-runbook.md,TODO.md | tests=updated | verify=npm run lint;npm run test:unit;npm run test:e2e | risks=the fallback textarea path is intentionally retained for resilience; if future work removes it, update E2E helpers and runbook editor-mode notes together
- 2026-02-17T17:11:11Z | AUTH-SIGNUP-IMPLEMENTATION | files=src/auth/account-store.ts,src/auth/account-service.ts,src/backend/routes/auth-routes.ts,src/backend/api-app.ts,src/backend/routes/workspace-routes.ts,tests/account-service.test.ts,tests/auth-account-creation.integration.test.ts,docs/local-first-runbook.md,TODO.md | tests=added/updated | verify=npm run lint;npm run test:unit;npm run test:e2e | risks=current signup path stores account credentials locally in file-backed JSON for local-first MVP; production rollout should add session management + stronger account lifecycle controls
- 2026-02-17T20:07:36Z | P1-05 | files=src/problems/problem-spec-v2.ts,src/problems/seed-problem-pack.ts,src/problems/suggest-topic-spec-v2.ts,src/problems/hint-leakage-lint.ts,src/problems/card-verification-pipeline.ts,src/problems/problem-bank-lint.ts,src/problems/problem-review-queue.ts,src/backend/workspace/problem-workspace-service.ts,src/backend/routes/problem-routes.ts,tests/problem-spec-v2.validation.test.ts,tests/hint-leakage-lint.test.ts,tests/card-verification-pipeline.test.ts,tests/problem-review-queue.test.ts,tests/problem-verify-card.endpoint.integration.test.ts,TODO.md | tests=added/updated | verify=npm run test:unit;npm run lint | risks=verification status and diagnostics are in-memory process state; multi-instance deployments need persistent verification storage and workflow orchestration
- 2026-02-17T21:11:48Z | PAPER-GENERATION-ANTHROPIC-LOOP | files=src/problems/paper-problem-generation.ts,src/runtime/runtime-execution.ts,src/problems/card-verification-pipeline.ts,src/backend/routes/problem-routes.ts,.env.example,package.json,package-lock.json,tests/problem-generate-from-paper.endpoint.integration.test.ts,TODO.md | tests=added/updated | verify=npm run test:unit;npm run lint | risks=paper ingestion currently performs lightweight HTML stripping and PDF text extraction; production-scale paper parsing quality and persistent generation experiment tracking should be added before wide rollout
